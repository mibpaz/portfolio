          ...
          <Container title="Frameworks">
            <SkillsGrid>
              {skills.frameworks.map((framework) => (
                <motion.div key={framework.name} variants={childVariants} className="flex flex-row items-center gap-2">
          ...
          <Container title="Tools and Technologies">
            <SkillsGrid>
              {skills.tools.map((tool) => (
                <motion.div key={tool.name} variants={childVariants} className="flex flex-row items-center gap-2">
                  <tool.icon size={24} />
                  <p>{tool.name}</p>
                </motion.div>
              ))}
          ...
          <Container title="Languages">
            <SkillsGrid>
              {skills.languages.map((language) => (
                <motion.div key={language.name} variants={childVariants} className="flex flex-row items-center gap-2">
          ...
          <Container title="Other stuff in my bag">
            <SkillsGrid>
              {skills.misc.map((misc) => (
                <motion.div key={misc.name} variants={childVariants} className="flex flex-row items-center gap-2">
                  <misc.icon size={24} />
                  <p>{misc.name}</p>
                </motion.div>
              ))}
